...
############################################################ Choose test data directory
   ...
   if [[ $# -ne 2 ]]; then
      echo "Invocations:  gradeC03.sh [-m1 | -m2 | -final | -ec] [name of submitted tar file]"
   ...
   if [[ $1 == "-m1" ]]; then
      ...
      echo "Running tests for final submission, except extra-credit."
      testDataDir="./final"
   elif [[ $1 == "-ec" ]]; then
      echo "Running all tests, including the extra-credit test."
      testDataDir="./final"
   else
      echo "Unknown option: $1"
      exit 2
...
############################################################ Extra-credit test
   ...
   if [[ $1 == "-ec" ]]; then
	   ...
	   # Prepare for EC test (based on ftest07.asm)
	   echo >> $testLog
	   echo "Begin testing for extra credit." >> $testLog
	   echo "  EC will only awarded if this is entirely correct." >> $testLog
	   
	   ecTestFile=$testDataDir/"ecTest.asm"
	   ecObjectFile="ecObject.o"
	   ecObjectRef=$testDataDir/"ecTest.o"
	   
	   # write-protect test files
	   chmod a-w $ecTestFile
	   chmod a-w $ecObjectRef
	   
	   # Run assembler on test file
	   timeout -s SIGKILL 30 ./$exeName $ecTestFile $ecObjectFile >> $testLog
		if [[ $timeoutStatus -eq 124 || $timeoutStatus -eq 137 ]]; then
			echo "The test of your solution timed out after 30 seconds." >> $testLog
			echo "Valgrind testing will NOT be done." >> $testLog
			killed="yes"
      ...
      # remove write-protection from test files
	   ...
	   # Verify existence of a nonempty student object file, and check results.
	   if [ ! -s $ecObjectFile ]; then
	      echo "The object file $ecObjectFile was not produced or is empty" >> $testLog
	      echo >> $testLog
	   else
	      # run comparator
	      echo "Running compare on $ecObjectFile and $ecObjectRef" >> $testLog
	      ./compare 14 $ecObjectFile $ecObjectRef >> $testLog
	      echo >> $testLog
	   fi
   fi
